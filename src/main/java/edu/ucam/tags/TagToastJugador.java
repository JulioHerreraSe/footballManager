...
public class TagToastJugador extends TagSupport{
	private static final long serialVersionUID = 1L;

	@Override
	public int doStartTag() throws JspException {
		if(pageContext.getServletContext().getAttribute("MSGJ") != null) {
			try {
				...
				pageContext.getOut().print("<p>"+pageContext.getServletContext().getAttribute("MSGJ")+"</p>");
				pageContext.getOut().print("</div>");
				pageContext.getOut().print("</div>");
				pageContext.getOut().print("</div>");
			} catch (IOException e) {
			...
			pageContext.getServletContext().setAttribute("MSGJ", null);
		}
