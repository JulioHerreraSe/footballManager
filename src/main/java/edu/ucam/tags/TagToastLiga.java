...
public class TagToastLiga extends TagSupport {
	private static final long serialVersionUID = 1L;

	@Override
	public int doStartTag() throws JspException {
		if(pageContext.getServletContext().getAttribute("MSGL") != null) {
			try {
				...
				pageContext.getOut().print("<p>"+pageContext.getServletContext().getAttribute("MSGL")+"</p>");
				pageContext.getOut().print("</div>");
				pageContext.getOut().print("</div>");
				pageContext.getOut().print("</div>");
			} catch (IOException e) {
			...
			pageContext.getServletContext().setAttribute("MSGL", null);
		}
